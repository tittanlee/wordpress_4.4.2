...
switch ($ui['action']) {
    ...
    case 'post.new':
        ...
        if ( time() - strtotime($last_post) < 120 ){
            print_r(json_encode(array('error'=>1, 'msg'=>'兩次提交文章時間間隔太短，請稍候再來')));  
            exit();
        }
        ...
        if ( empty($title) || mb_strlen($title) > 50 ) {
            print_r(json_encode(array('error'=>1, 'msg'=>'標題不能為空，且小於50個字符')));  
            exit();
        }

        if ( empty($content) || mb_strlen($content) > 10000 || mb_strlen($content) < 10 ) {
            print_r(json_encode(array('error'=>1, 'msg'=>'文章內容不能為空，且介於10-10000字之間')));  
            exit();
        }

        if ( !empty($url) && mb_strlen($url) > 200 ) {
            print_r(json_encode(array('error'=>1, 'msg'=>'來源鏈接不能大於200個字符')));  
            exit();
        }

        if( !empty($url) ){
            $content .= '<p>來源：<a href="'.$url.'" target="_blank">'.$url.'</a></p>';
        }
        ...
        if( !empty($posttitle) ){
            print_r(json_encode(array('error'=>1, 'msg'=>'標題 '. $posttitle .' 已存在')));  
            exit();
        }
        ...
        // fail
        if (!$in_id) { 
            print_r(json_encode(array('error'=>1, 'msg'=>'投稿失敗，請稍後再試')));  
            exit();
        }
        ...
        if( _hui('tougao_mail_send') ){
            wp_mail(_hui('tougao_mail_to'), '站長，有新投稿：'.$title, $content);
        }

        print_r(json_encode(array('error'=>0, 'msg'=>'投稿成功，站長審核中...')));  
        exit();
    ...
    case 'posts':
        ...
        if( $ui['first'] ){
            $printr['menus'] = array(
                ...
                array('name' => 'publish', 'title' => '已發佈', 'count' => u_post_count('publish') ),
                array('name' => 'future', 'title' => '定時', 'count' => u_post_count('future') ),
                array('name' => 'pending', 'title' => '待審', 'count' => u_post_count('pending') ),
	...
	case 'info.edit':
        ...
        if( !$ui['nickname'] || ($ui['nickname'] && sstrlen($ui['nickname'])>12) || ($ui['nickname'] && sstrlen($ui['nickname'])<2) ) {  
            print_r(json_encode(array('error'=>1, 'msg'=>'暱稱不能為空且限制在2-12字內')));  
            exit();  
        }

        /*if( !$ui['email'] ) {  
            print_r(json_encode(array('error'=>1, 'msg'=>'郵箱不能為空')));  
            exit();  
        }
        ...
        if( $ui['url'] && (!preg_match("/^((http|https)\:\/\/)([a-z0-9-]{1,}.)?[a-z0-9-]{2,}.([a-z0-9-]{1,}.)?[a-z0-9]{2,}$/", $ui['url']) || sstrlen($ui['url'])>100) ){
            print_r(json_encode(array('error'=>1, 'msg'=>'網址格式錯誤')));  
            exit(); 
        }

        if( $ui['weibo'] && (!preg_match("/^((http|https)\:\/\/)([a-z0-9-]{1,}.)?[a-z0-9-]{2,}.([a-z0-9-]{1,}.)?[a-z0-9]{2,}$/", $ui['weibo']) || sstrlen($ui['weibo'])>100) ){
            print_r(json_encode(array('error'=>1, 'msg'=>'微博格式錯誤')));  
            exit(); 
        }

        if( $ui['qq'] && !preg_match("/^[1-9]\d{4,13}$/", $ui['qq']) ){
            print_r(json_encode(array('error'=>1, 'msg'=>'QQ格式錯誤')));  
            exit(); 
        }

        if( $ui['weixin'] && sstrlen($ui['weixin'])>30 ) {  
            print_r(json_encode(array('error'=>1, 'msg'=>'微信字數過長，限制在30字內')));  
            exit();  
        }
...
function u_get_thumbnail_src() {  
    ...
    $content = user_get_post_thumbnail();  
    preg_match_all('/<img.*?(?: |\\t|\\r|\\n)?src=[\'"]?(.+?)[\'"]?(?:(?: |\\t|\\r|\\n)+.*?)?>/sim', $content, $strResult, PREG_PATTERN_ORDER);  
    return $strResult[1][0];
}

function user_get_post_thumbnail($size = 'thumbnail', $class = 'thumb') {
	$html = '';
