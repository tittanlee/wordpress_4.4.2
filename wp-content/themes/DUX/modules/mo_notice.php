...
?>
	<div class="widget widget-tops">
		<ul class="widget-nav">
			<?php if( $s_notice ){ ?><li<?php echo ($s_notice) ? ' class="active"' : '' ?>><?php echo _hui('site_notice_title') ? _hui('site_notice_title') : '網站公告' ?></li><?php } ?>
			<?php if( $s_user ){ ?><li<?php echo ($s_user && !$s_notice ) ? ' class="active"' : '' ?>>會員中心</li><?php } ?>
		</ul>
				...
				<li class="item item-02<?php echo ($s_user && !$s_notice) ? ' active' : '' ?>">
					...
					?>
						...
						<ul>
							...
							<li><a href="<?php echo mo_get_user_page() . '#comments' ?>">我的評論</a></li>
							<li><a href="<?php echo mo_get_user_page() . '#info' ?>">修改資料</a></li>
							<li><a href="<?php echo mo_get_user_page() . '#password' ?>">修改密碼</a></li>
						</ul>
					<?php }else{ ?>
						<h4>需要登錄才能進入會員中心</h4>
						<p>
							<a href="javascript:;" class="btn btn-primary signin-loader">立即登錄</a>
							<a href="javascript:;" class="btn btn-default signup-loader">現在註冊</a>
						</p>
					<?php } ?>
				</li>
			<?php } ?>
