...
<div class="title" id="comments">
	<h3><?php echo _hui('comment_title') ?> <?php echo $count_t ? '<b>'.$count_t.'</b>':'<small>搶沙發</small>'; ?></h3>
</div>
<div id="respond" class="no_webshot">
	<?php if ( get_option('comment_registration') && !is_user_logged_in() ) { ?>
	<div class="comment-signarea">
		<h3 class="text-muted">評論前必須登錄！</h3>
		<p>
			<a href="javascript:;" class="btn btn-primary signin-loader">立即登錄</a> &nbsp; 
			<a href="javascript:;" class="btn btn-default signup-loader">註冊</a>
		</p>
	</div>
	<?php }elseif( get_option('close_comments_for_old_posts') && $closeTimer > get_option('close_comments_days_old') ) { ?>
	<h3 class="title">
		<strong>文章評論已關閉！</strong>
	</h3>
	<?php }else{ ?>
		...
		<div class="comt">
				...
				<?php 
					if ( is_user_logged_in() ) {
						...
						if ( !empty($comment_author) ){
							...
							echo '<p><a href="javascript:;" evt="comment-user-change">更換</a></p>';
						}
					}
				?>
			...
			<div class="comt-box">
				...
				<div class="comt-ctrl">
					...
					<button type="submit" name="submit" id="submit" tabindex="5"><?php echo _hui('comment_submit_text') ? _hui('comment_submit_text') : '提交評論' ?></button>
			...
			<?php if ( !is_user_logged_in() ) { ?>
				<?php if( get_option('require_name_email') ){ ?>
					<div class="comt-comterinfo" id="comment-author-info" <?php if ( !empty($comment_author) ) echo 'style="display:none"'; ?>>
						<ul>
							<li class="form-inline"><label class="hide" for="author">暱稱</label><input class="ipt" type="text" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" tabindex="2" placeholder="暱稱"><span class="text-muted">暱稱 (必填)</span></li>
