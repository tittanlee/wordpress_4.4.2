...
function readers_wall( $outer='1',$timer='3',$limit='100' ){
	...
	foreach ($counts as $count) {
		...
		if( $i < 4 ){
			$type .= '<a class="item-top item-'.$i.'" target="_blank" href="'. $c_url . '"><h4>【'.$tt.'】<small>評論：'. $count->cnt . '</small></h4>'.str_replace(' src=', ' data-src=', get_avatar( $count->comment_author_email, $size = '36' , AVATAR_DEFAULT )).'<strong>'.$count->comment_author.'</strong>'.$c_url.'</a>';
		}else{
			$type .= '<a target="_blank" href="'. $c_url . '" title="【'.$tt.'】評論：'. $count->cnt . '">'.str_replace(' src=', ' data-src=', get_avatar( $count->comment_author_email, $size = '36' , AVATAR_DEFAULT )).$count->comment_author.'</a>';
		}
