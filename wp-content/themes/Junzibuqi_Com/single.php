	...
	<div class="content">
		...
		<header class="article-header">
			...
			<div class="article-meta">
				...
				<span class="item"><?php echo '分類：';the_category(' / '); ?></span>
				...
				<span class="item"><?php edit_post_link('[編輯]'); ?></span>
			</div>
		</header>
		...
		?>
		<div class="post-like">
				<a href="javascript:;" data-action="ding" data-id="<?php the_ID(); ?>" class="favorite<?php if(isset($_COOKIE['bigfa_ding_'.$post->ID])) echo ' done';?>" <?php if(isset($_COOKIE['bigfa_ding_'.$post->ID])){echo 'title="請勿重複點贊，每個小時內只能點贊一次！"';} else {echo 'title="感謝您的點贊！"';}?>><i class="fa fa-heart"></i>點贊 (<span class="count"><?php if( get_post_meta($post->ID,'bigfa_ding',true) ){
                    echo get_post_meta($post->ID,'bigfa_ding',true);
                 } else {
                    echo '0';
                 }?></span>)</a><span class="or">or</span><a class="paizhuan" href="javascript:;" data-action="cai" data-id="<?php the_ID(); ?>" class="paizhuan post-paizhuan<?php if(isset($_COOKIE['junke_cai_'.$post->ID])) echo ' done';?>" <?php if(isset($_COOKIE['junke_cai_'.$post->ID])){echo 'title="您已經拍過磚了！"';} else {echo 'title="磚頭很硬的~求不拍~~~~！"';}?>><i class="fa fa-heart-o"></i>拍磚 (<span class="count"><?php if( get_post_meta($post->ID,'junke_cai',true) ){
		...
		<div class="article-tags"><?php the_tags('標籤：','',''); ?></div>
		<?php _the_ads($name='ads_post_02', $class='asb-post asb-post-02') ?>
		<nav class="pager" role="navigation">
			<li class="previous">
				<?php $prev_post = get_previous_post(); get_previous_post() ? print '<a title="上一篇：'.$prev_post->post_title.'" href="'.get_permalink( $prev_post->ID ).'">上一篇：'.$prev_post->post_title.'</a>' : print '<a>很抱歉，這已經是最後的一篇文章了！</a>';?>
			</li>
			<li class="next">
				<?php $next_post = get_next_post(); get_next_post() ? print '<a title="下一篇：'.$next_post->post_title.'" href="'.get_permalink( $next_post->ID ).'">下一篇：'.$next_post->post_title.'</a>' : print '<a>很抱歉，這已經是最新的一篇文章了！</a>';?>
			</li>
		</nav>
	<div class="pads">
<?php
