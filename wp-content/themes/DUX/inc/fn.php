...
function _get_post_views($before = '閱讀(', $after = ')') {
	global $post;
	$post_ID = $post->ID;
...
function _get_post_comments($before = '評論(', $after = ')') {
...
function _comment_mail_notify($comment_id) {
	...
	if ($parent_id != '' && $spam_confirmed != 'spam' && $notify == '1') {
		...
		$subject = 'Hi，您在 [' . get_option("blogname") . '] 的留言有人回復啦！';
...
function _comment_add_checkbox() {
	echo '<label for="comment_mail_notify" class="checkbox inline hide" style="padding-top:0"><input type="checkbox" name="comment_mail_notify" id="comment_mail_notify" value="comment_mail_notify" checked="checked"/>有人回復時郵件通知我</label>';
}
