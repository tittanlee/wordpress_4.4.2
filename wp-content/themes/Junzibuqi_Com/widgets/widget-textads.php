...
class widget_ui_textads extends WP_Widget {
	function widget_ui_textads() {
		$widget_ops = array( 'classname' => 'widget_ui_textasb', 'description' => '顯示一個文本特別推薦' );
		$this->WP_Widget( 'widget_ui_textads', 'D-特別推薦', $widget_ops );
	}
	...
	function form($instance) {
		$defaults = array( 
			...
			'tag' => '吐血推薦', 
			'content' => '君子不器（Junzibuqi.Com）致力於整合國內外各種優秀的WordPress資源，並且對國外優秀的WordPress主題插件進行漢化並分享。', 
...
?>
		...
		<p>
			<label>
				...
				<select style="width:100%;" id="<?php echo $this->get_field_id('style'); ?>" name="<?php echo $this->get_field_name('style'); ?>" style="width:100%;">
					<option value="style01" <?php selected('style01', $instance['style']); ?>>藍色</option>
					<option value="style02" <?php selected('style02', $instance['style']); ?>>橘紅色</option>
					<option value="style03" <?php selected('style03', $instance['style']); ?>>綠色</option>
		...
		<p>
			<label>
				<input style="vertical-align:-3px;margin-right:4px;" class="checkbox" type="checkbox" <?php checked( $instance['blank'], 'on' ); ?> id="<?php echo $this->get_field_id('blank'); ?>" name="<?php echo $this->get_field_name('blank'); ?>">新打開瀏覽器窗口
			</label>
		</p>
<?php
	}
