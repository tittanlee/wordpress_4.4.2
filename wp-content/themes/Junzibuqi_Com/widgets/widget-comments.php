...
class widget_ui_comments extends WP_Widget {
	function widget_ui_comments() {
		$widget_ops = array( 'classname' => 'widget_ui_comments', 'description' => '顯示網友最新評論（頭像+名稱+評論）' );
		$this->WP_Widget( 'widget_ui_comments', 'D-最新評論', $widget_ops );
	}
...
function mod_newcomments( $limit,$outpost,$outer ){
	...
	foreach ( $comments as $comment ) {
		$output .= '<li><a href="'.get_comment_link($comment->comment_ID).'" title="'.$comment->post_title.'上的評論">'._get_the_avatar($user_id=$comment->user_id, $user_email=$comment->comment_author_email).' <strong>'.$comment->comment_author.'</strong> '._get_time_ago( $comment->comment_date_gmt ).'說：<br>'.str_replace(' src=', ' data-original=', convert_smilies(strip_tags($comment->com_excerpt))).'</a></li>';
	}
