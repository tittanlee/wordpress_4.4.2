...
case 'resetpass' :
	...
	if( strlen($_POST['pass1'])<6 ) {  
		$errors->add( 'password_reset_mismatch2', '密碼至少6位。' );
    }
...
<div class="content-wrap">
	<div class="content resetpass">
		...
		<ul class="resetpasssteps">
			<li<?php echo $classactive1 ?>>獲取密碼重置郵件<span class="glyphicon glyphicon-chevron-right"></span></li>
			<li<?php echo $classactive2 ?>>設置新密碼<span class="glyphicon glyphicon-chevron-right"></span></li>
			<li<?php echo $classactive3 ?>>成功修改密碼</li>
		</ul>
		...
		<form action="<?php echo esc_url( mo_get_user_rp() . '?action=lostpassword' ); ?>" method="post">
			<?php errormsg($errors); ?>
			<h3>填寫用戶名或郵箱：</h3>
			<p><input type="text" name="user_login" class="form-control input-lg" placeholder="用戶名或郵箱" autofocus></p>
			<p><input type="submit" value="獲取密碼重置郵件" class="btn btn-block btn-primary btn-lg"></p>
		</form>
		<?php 
			}else{ 
				echo '<h3><span class="text-success">已向註冊郵箱發送郵件！</span></h3>';
				echo '<p>去郵箱查收郵件並點擊重置密碼鏈接</p>';
			}
		} ?>
		...
		<form action="" method="post">
			<?php errormsg($errors); ?>
			<h3>設置新密碼：</h3>
			<p><input type="password" name="pass1" class="form-control input-lg" placeholder="輸入新密碼" autofocus></p>
			<h5>重複新密碼：</h5>
			<p><input type="password" name="pass2" class="form-control input-lg" placeholder="重複新密碼"></p>
			<p><input type="submit" value="確認提交" class="btn btn-block btn-primary btn-lg"></p>
		</form>
		<?php } ?>

		<?php if( $classactive3 ){ ?>
		<form>
			<h3><span class="text-success"><span class="glyphicon glyphicon-ok-circle"></span> 恭喜，您的密碼已重置！</span></h3>
			<p> &nbsp; </p>
			<p class="text-center"><a class="btn btn-success btn-lg" href="<?php echo get_bloginfo('url') ?>">回首頁</a></p>
		</form>
		<?php } ?>
